...
- **チェイサーのステーションキーピング(デッドバンド制御)**: チェイサー(カメラ)は編隊誤差が制御ボックス(画面の S/K BOX、スタンドオフの ±18%)を超えた時だけスラスタを噴き、ボックス内では慣性飛行。画面上でデブリが漂って見えるのは CW の自然な相対運動そのもの。消費 Δv は燃料として表示され、REALISM/EXPERT では枯渇 = 失敗
